...
github.name <- "maxdor"
email.address <- "maxdor@berkeley.edu"
...
# a random permutation of the even numbers from 2 to 222
  set.seed(31415)
v <- seq(2, 222, by = 2)
v <- v[sample(1:length(v), length(v))]
...
set.seed(2718)
w <- rep(1:13, each = 4)
w <- w[sample(1:length(w), length(w))]
...
# Each vector of length 100 of Poisson (hint:rpois) random variables with mean 5
  set.seed(71)
l <- lapply(1:12, function (x) {rpois(100, lambda = 5)})
...
f3 <- family[grep("^T", as.character(family$name)),]
...
plot(iris$Petal.Width~iris$Petal.Length, col = rep(2:4, each = 50), ylab = "Petal Width", xlab = "Petal Length")
...
plot((iris$Sepal.Length/iris$Petal.Length),
     col = rep(2:4, each = 50), ylab = "sepal length / petal length", xlab = "Index")
...
plot(infants$bwt~infants$gestation, pch = "*", col = "red", xlab = "Gestation Time", ylab = "Birthweight")
abline(v = 259, col = "dodgerblue")
...
# Add x-axis labels
hist(infants$age, freq = F)
points(density(infants$age[-which(is.na(infants$age))]), type = 'l', col = "blue")
...
max.petal.width <- as.numeric(by(iris$Petal.Width, iris$Species, max))
...
dice_sum <- function(k, B){
  rolls <- rep(0, B)
  for (i in 1:B) {
    rolls[i] <- sum(replicate(k, trunc(runif(1, 1, 7))))
  }
  return(rolls)
}
...
sim1 <- dice_sum(2, 20)
sim2 <- dice_sum(2, 100)
sim3 <- dice_sum(2, 1000)
sim4 <- dice_sum(2, 5000)

ave.diceRoll <- c(mean(sim1), mean(sim2), mean(sim3), mean(sim4))
sd.diceRoll <- c(sd(sim1), sd(sim2), sd(sim3), sd(sim4))

hist(sim1, xlab = "sum of dice roll", main = "Histogram for B=20")
hist(sim2, xlab = "sum of dice roll", main = "Histogram for B=100")
hist(sim3, xlab = "sum of dice roll", main = "Histogram for B=1000")
hist(sim4, xlab = "sum of dice roll", main = "Histogram for B=5000")
